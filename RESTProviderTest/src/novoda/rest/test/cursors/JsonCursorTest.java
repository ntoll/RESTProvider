...
public class JsonCursorTest extends TestCase {
	...
	public void testEmptyJsonShouldReturnCursorWithCountZero() throws Exception {
		...
		JsonCursor cursor = new JsonCursor((String) "root")
	...
	public void testComplexJsonWithForeignKeys() throws Exception {
		...
		MockHttpResponse response = new MockHttpResponse(json);
		JsonCursor cursor = new JsonCursor("sets", true, "id").withForeignKey(
				"tracks").handleResponse(response);
		assertTrue(cursor.moveToFirst());
		assertEquals("Armin Van Buuren", cursor.getString(cursor
	...
	public void testComplexJsonWithForeignKeys2() throws Exception {
		...
		assertEquals("Armin Van Buuren", cursor.getString(cursor
				.getColumnIndexOrThrow("show_artist")));
		JsonCursor tree = cursor.getForeignCursor(0, "tracks");
		assertNotNull(tree);
		assertTrue(tree.moveToFirst());
		assertEquals("A State of Trance", tree.getString(tree
				.getColumnIndexOrThrow("artist")));
